...
pub(crate) async fn get_columns(pool:&SqlitePool) -> DbResult<Vec<Column>> {
    ...
    const SQL2: &str = "SELECT cc.column_id, cards.id, cards.title, cards.description \
        ...
        WHERE \
            cards.id = cc.card_id \
        ORDER BY \
            cc.column_id ASC, cc.card_position ASC";
...
async fn insert_card_position(
    ...
    updata_card_position(tx, column_id, position, |pos| pos + 1).await?;
...
async fn delete_card_position(
    ...
    updata_card_position(tx, column_id, positioin, |pos| pos - 1).await?;
    Ok(())
}

async fn updata_card_position(
    tx:&mut Transaction<'_, Sqlite>,
    column_id:i64,
    start_position:i64,
    mut new_posotion:impl FnMut(i64) -> i64,
) -> DbResult<()> {
    ...
    std::mem::drop(rows); // ドロップ（メモリ解放）を明示的に行う
